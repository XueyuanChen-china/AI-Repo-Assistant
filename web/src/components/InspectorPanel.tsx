...
export function InspectorPanel({ openFile, inspectorMode, diffPreview, onModeChange }: InspectorPanelProps) {
  const subtitle =
    inspectorMode === 'diff' && diffPreview
      ? `${diffPreview.path} · suggested change preview`
      : openFile
        ? `${openFile.path} · ${openFile.language}`
        : 'Pick a file to inspect its contents'
  ...
  return (
    ...
    >
      ...
      {inspectorMode === 'diff' && diffPreview ? (
        ...
        <p className="panel-empty">Open a file from the repository panel to inspect it here.</p>
      )}
    </PanelCard>
  )
}
